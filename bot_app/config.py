...
class Settings:
    ...
    # Бесплатный расклад «1 карта»
    enable_free_one_card: bool
    free_cards_per_day: int
    # Telegram Stars (альтернатива USDT)
    stars_payments: bool
    stars_tier5: int
    stars_tier10: int
    # Для ссылок t.me/.../start=… (без @)
    bot_username: str | None
    # Папка с PNG: free.png, three.png, extended.png (опционально)
    ...
    def from_env(cls) -> Settings:
        ...
        sp = _bool_env("STARS_PAYMENTS", False)
        s5 = _int_env("STARS_TIER5", 50)
        s10 = _int_env("STARS_TIER10", 100)
        ...
        return cls(
            ...
            enable_free_one_card=_bool_env("ENABLE_FREE_ONE_CARD", True),
            free_cards_per_day=max(1, _int_env("FREE_CARDS_PER_DAY", 1)),
            stars_payments=sp,
            stars_tier5=s5,
            stars_tier10=s10,
            bot_username=os.environ.get("BOT_USERNAME", "").strip() or None,
            cards_images_dir=os.environ.get("CARDS_IMAGES_DIR", "").strip() or None,
            upsell_note=os.environ.get(
